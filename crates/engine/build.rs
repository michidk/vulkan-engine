...
fn main() {
    ...
    for path in paths {
        ...
        if let Some(ext) = path.extension() {
            if ext == "hlsl" {
                println!("Compiling shader {}", &path.file_name().unwrap().to_str().unwrap());
...
fn compile_shader(target_dir: &std::path::PathBuf, source_path: &std::path::PathBuf, shader_type: &str) -> std::process::Output {
    let output_path = target_dir.join(format!("{}-{}.spv", &source_path.file_prefix().unwrap().to_str().unwrap(), shader_type));
