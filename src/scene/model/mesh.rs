...
impl Drop for Mesh {
    fn drop(&mut self) {
        ...
        if let Some(rtx_data) = &self.rtx_data {
            unsafe{rtx_data.rtx_ext.destroy_acceleration_structure(rtx_data.acc_struct, None)};
            self.allocator.destroy_buffer(rtx_data.buffer, &rtx_data.buffer_alloc);
        }
    }
}
...
impl Mesh {
    pub fn bake(
        ...
        let extra_flags = if rtx_ext.is_some() { vk::BufferUsageFlags::SHADER_DEVICE_ADDRESS | vk::BufferUsageFlags::ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_KHR } else { vk::BufferUsageFlags::empty() };

        let vertex_buffer_size = mesh_data.vertices.len() * size_of::<Vertex>();
        let (vertex_buffer, vertex_buffer_alloc) =
            allocator.create_buffer(vertex_buffer_size as u64, vk::BufferUsageFlags::VERTEX_BUFFER | vk::BufferUsageFlags::TRANSFER_DST | extra_flags, gpu_allocator::MemoryLocation::GpuOnly);
        ...
        let (index_buffer, index_buffer_alloc) =
            allocator.create_buffer(index_buffer_size as u64, vk::BufferUsageFlags::INDEX_BUFFER | vk::BufferUsageFlags::TRANSFER_DST | extra_flags, gpu_allocator::MemoryLocation::GpuOnly);
        ...
        let rtx_data = rtx_ext.map(|rtx_ext| {
            let (acc, acc_buf, acc_alloc) = uploader.enqueue_acc_struct_build(rtx_ext.clone(), vertex_buffer, index_buffer, &submeshes, mesh_data.vertices.len() as u32);
            MeshRtxData {
                acc_struct: acc,
                buffer: acc_buf,
