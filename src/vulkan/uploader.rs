...
impl Uploader {
    ...
    fn find_staging_buffer(&mut self, size: u64) -> usize {
        ...
        let (buffer, alloc) = self
            .allocator
            .create_buffer(new_size, vk::BufferUsageFlags::TRANSFER_SRC, MemoryLocation::CpuToGpu);
    ...
    pub fn enqueue_acc_struct_build(&mut self, rtx_ext: Rc<khr::AccelerationStructure>, vertex_buffer: vk::Buffer, index_buffer: vk::Buffer, submeshes: &[(u32, u32)], vertex_count: u32) -> (vk::AccelerationStructureKHR, vk::Buffer, SubAllocation) {
        ...
        let vertex_buffer_addr = unsafe { self.device.get_buffer_device_address(&vk::BufferDeviceAddressInfo::builder().buffer(vertex_buffer)) };
        let index_buffer_addr = unsafe { self.device.get_buffer_device_address(&vk::BufferDeviceAddressInfo::builder().buffer(index_buffer)) };
        
        let geometries = [
            vk::AccelerationStructureGeometryKHR::builder()
                .geometry_type(vk::GeometryTypeKHR::TRIANGLES)
                .geometry(vk::AccelerationStructureGeometryDataKHR{ triangles: vk::AccelerationStructureGeometryTrianglesDataKHR::builder()
                    .vertex_format(vk::Format::R32G32B32_SFLOAT)
                    .vertex_data(vk::DeviceOrHostAddressConstKHR{device_address: vertex_buffer_addr})
                    .vertex_stride(size_of::<Vertex>() as u64)
                    .max_vertex(vertex_count-1)
                    .index_type(vk::IndexType::UINT32)
                    .index_data(vk::DeviceOrHostAddressConstKHR{device_address: index_buffer_addr})
                    .build()}
                )
                .build()
        ];
    ...
    pub fn submit_uploads(&mut self, queue: vk::Queue) {
        ...
        unsafe {
            let mem_barrier = vk::MemoryBarrier::builder()
                .src_access_mask(vk::AccessFlags::TRANSFER_WRITE | vk::AccessFlags::ACCELERATION_STRUCTURE_WRITE_KHR)
                .dst_access_mask(vk::AccessFlags::empty())
                .build();
            self.device.cmd_pipeline_barrier(
                command_buffer,
